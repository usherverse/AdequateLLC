-- ==============================================================================
-- ADEQUATE CAPITAL LMS - SUPABASE V3 SCHEMA UPGRADE
...
-- ==============================================================================
-- 1. BASE TABLES & CONSTRAINTS (Sections 1, 4, 5, 8)
-- ==============================================================================

-- Create Tables (assuming they may exist, using IF NOT EXISTS or ALTERs handled via CREATE)
...
-- Helper function to get current worker role
...
SELECT 
    ...
    CASE 
        WHEN public.get_auth_role() = 'Viewer / Auditor' THEN 'MASKED-' || right(id_no, 4) 
        ELSE id_no 
    END AS id_no,
    CASE 
...
-- Financial summary view for external reporting
...
-- Rule: Prevents workers from modifying their own role
...
CREATE POLICY "Collections View Overdue Loans" ON public.loans FOR SELECT USING (get_auth_role() = 'Collections Officer' AND (days_overdue > 0 OR status = 'Overdue'));
...
-- Rule: Written off or Settled loans cannot be modified, unless Admin.
CREATE POLICY "Update loans protection" ON public.loans FOR UPDATE USING (
    (get_auth_role() = 'admin') OR 
    (get_auth_role() = 'Loan Officer' AND assigned_officer = auth.uid() AND status NOT IN ('Written off', 'Settled'))
);

-- PAYMENTS (Immutable once allocated)
CREATE POLICY "View Payments" ON public.payments FOR SELECT USING (get_auth_role() IS NOT NULL);
CREATE POLICY "Insert Payments" ON public.payments FOR INSERT WITH CHECK (get_auth_role() IN ('admin', 'Finance', 'Collections Officer', 'Loan Officer'));
...
-- Deletion blocked completely fallback (only Postgres Superuser can override)
...
CREATE POLICY "View interactions" ON public.interactions FOR SELECT USING (
    ...
    (get_auth_role() = 'Asset Recovery' AND (SELECT status FROM loans WHERE interactions.loan_id = loans.id) = 'Written off')
);
...
CREATE POLICY "Insert audit log" ON public.audit_log FOR INSERT WITH CHECK (TRUE); -- Usually triggers handle this bypassing RLS, but safe measure
...
-- In Supabase, you would call this function daily via pg_cron extension:
-- SELECT cron.schedule('calculate_penalties', '0 0 * * *', 'SELECT process_daily_loan_updates()');
CREATE OR REPLACE FUNCTION process_daily_loan_updates() RETURNS VOID AS $$
BEGIN
    -- Update days overdue for Active loans
    UPDATE public.loans 
    ...
    -- Apply penalty (example 1% daily), cap at 3650
    UPDATE public.loans
...
BEGIN
    IF NEW.status = 'Allocated' AND NEW.loan_id IS NOT NULL THEN
        ...
        -- Simplified abstract: Custom logic needed in JS to allocate partial exact amounts, but mark early ones paid.
        UPDATE public.loan_schedules SET status = 'paid', amount_paid = amount_due, paid_date = NOW() 
        WHERE loan_id = NEW.loan_id AND status != 'paid' AND amount_due <= NEW.amount
        RETURNING id;
